...
COINS_PER_USDT   = 100 / 1.5   # 100 коинов = 1.5 USDT
# pending invoices: invoice_id -> {user_id, coins}
_cb_pending: dict = {}
...
# ─── CRYPTOBOT: POST /create_crypto_invoice ───────────────────────────────────
async def http_create_crypto_invoice(request: web.Request) -> web.Response:
    if request.method == "OPTIONS":
        return web.Response(status=204, headers=CORS)
    try:
    ...
    try:
        ...
        return web.json_response({"error": "coins must be int"}, status=400, headers=CORS)

    amount = round(coins / COINS_PER_USDT, 2)
        ...
        async with aiohttp.ClientSession() as session:
            resp = await session.post(
                ...
                json={
                    "asset":       "USDT",
                    "amount":      str(amount),
                    "description": f"{coins} 🟡 золотых коинов • FLEEP GIFT",
    ...
    if not data.get("ok"):
        err = data.get("error", {})
        logger.error(f"CryptoBot error: {err}")
        return web.json_response({"error": str(err)}, status=500, headers=CORS)

    invoice = data["result"]
    inv_id  = invoice["invoice_id"]
    pay_url = invoice["bot_invoice_url"]

    _cb_pending[inv_id] = {"user_id": int(user_id), "coins": coins}
    logger.info(f"CryptoBot invoice created: id={inv_id} user={user_id} coins={coins} amount={amount}")

    return web.json_response({"pay_url": pay_url, "invoice_id": inv_id, "amount": amount}, headers=CORS)


# ─── CRYPTOBOT: POST /cryptobot_webhook ───────────────────────────────────────
async def http_cryptobot_webhook(request: web.Request) -> web.Response:
    ...
    if token_header != CRYPTOBOT_TOKEN:
        logger.warning("CryptoBot webhook: bad token")
    ...
    payload = invoice.get("payload", "")  # "user_id:coins"

    logger.info(f"CryptoBot webhook paid: inv_id={inv_id} payload={payload}")

    # Parse payload
    try:
        user_id_str, coins_str = payload.split(":")
        user_id = int(user_id_str)
        coins   = int(coins_str)
    except Exception:
        logger.error(f"Bad webhook payload: {payload}")
        return web.Response(text="ok")

    # Remove from pending
    _cb_pending.pop(inv_id, None)

    # Credit coins
    conn = sqlite3.connect(DB_PATH)
    conn.execute("UPDATE users SET gold_coins = gold_coins + ? WHERE user_id = ?", (coins, user_id))
    conn.commit()
    conn.close()
    logger.info(f"Credited {coins} coins to user {user_id}")
    ...
    if _app_bot:
        try:
            await _app_bot.send_message(
                ...
                text=(
                    f"✅ Оплата получена!\n"
                    f"💰 +{coins} 🟡 золотых коинов зачислено\n"
...
async def start_http():
    ...
    app_http.router.add_post("/create_crypto_invoice",  http_create_crypto_invoice)
    app_http.router.add_options("/create_crypto_invoice", http_create_crypto_invoice)
