...
def init_db():
    ...
    try:
        ...
        pass
    try:
        conn.execute("ALTER TABLE users ADD COLUMN silver_coins INTEGER NOT NULL DEFAULT 0")
    except Exception:
        pass
    conn.commit()
    conn.close()
...
def find_user_by_username(username: str):
    ...
    conn = sqlite3.connect(DB_PATH)
    row = conn.execute(
        "SELECT user_id, username, full_name FROM users WHERE LOWER(username)=?",
        (username,)
    ).fetchone()
    conn.close()
    return row
...
async def run():
    ...
    admin_conv = ConversationHandler(
        ...
        states={
            WAIT_BROADCAST_TEXT: [
                CallbackQueryHandler(admin_menu_choice, pattern=r"^adm_(broadcast|addbal)$"),
