...
# ─── USDT (TRC20) ОПЛАТА ──────────────────────────────────────────────────────
USDT_WALLET     = "TCg8oyR58dT4wWdP3X56TMXa8McYA31QcoTrc20"  # твой TRC20 адрес
USDT_PER_100    = 1.5          # 100 коинов = 1.5 USDT
USDT_CONTRACT   = "TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t"  # USDT TRC20 контракт
USDT_INVOICE_TTL = 30 * 60     # 30 минут на оплату

# Хранилище ожидающих USDT платежей: amount_str -> {user_id, coins, expires}
_usdt_pending: dict = {}
_usdt_counter: int  = 0        # для генерации уникальных сумм

# Railway/Render выставляют PORT сами, локально не используется
PORT = int(os.environ.get("PORT", 0))

# Публичный URL Railway-деплоя (задаётся переменной окружения BACKEND_URL)
# Например: https://fleep-gift-bot.up.railway.app
...
# Глобальный bot-объект, доступный HTTP-хендлерам
_app_bot = None
...
def verify_init_data(init_data: str) -> bool:
    try:
        # Пустой init_data — пропускаем (dev/test режим)
        if not init_data:
            logger.warning("verify_init_data: empty init_data, skipping check")
            return True

        # URL-decode на случай если фронтенд передал encoded строку
        ...
        if not hash_val:
            logger.warning("verify_init_data: no hash found in init_data")
            return False
        ...
        if not result:
            logger.warning("verify_init_data: HMAC mismatch")
        return result
    except Exception as e:
        logger.error(f"verify_init_data exception: {e}")
        return False
...
# ─── HTTP: POST /create_invoice ───────────────────────────────────────────────
# Тело запроса: { "user_id": 123, "stars": 100, "promo": "CODE" }
# Ответ:        { "invoice_url": "https://t.me/$invoices/..." }
async def http_create_invoice(request: web.Request) -> web.Response:
    ...
    if not user_id or not stars:
        return web.json_response({"error": "user_id and stars required"}, status=400, headers=CORS)
    ...
    try:
        ...
        return web.json_response({"error": "stars must be integer"}, status=400, headers=CORS)

    if not (MIN_STARS <= stars <= MAX_STARS):
        return web.json_response({"error": f"stars must be {MIN_STARS}..{MAX_STARS}"}, status=400, headers=CORS)
    ...
    coins = calc_coins(stars, promo)
    promo_valid = promo and promo in PROMO_CODES
    bonus_pct   = int(PROMO_CODES[promo] * 100) if promo_valid else 0

    title = f"⭐ {stars} → 🟡 {coins} коинов"
    desc  = f"🟡 {coins} золотых коинов для FLEEP GIFT"
    if promo_valid:
        title += f" (+{bonus_pct}%)"
        desc  += f" (+{bonus_pct}% по промокоду {promo})"

    payload = f"stars_{stars}_{coins}_{user_id}"

    try:
        # Получаем Application из глобальной переменной
        invoice_link = await _app_bot.create_invoice_link(
            title=title,
            description=desc,
            payload=payload,
            provider_token="",
        ...
        return web.json_response({"invoice_url": invoice_link}, headers=CORS)
    except Exception as e:
...
# ─── USDT TRC20: генерация уникальной суммы ───────────────────────────────────
def usdt_unique_amount(coins: int) -> str:
    """100 коинов = 1.5 USDT, добавляем микро-копейку для уникальности."""
    global _usdt_counter
    _usdt_counter += 1
    base = round(coins * USDT_PER_100 / 100, 2)
    # Добавляем уникальный суффикс: 0.000001 * counter (незаметно для юзера)
    unique = base + (_usdt_counter % 1000) * 0.000001
    return f"{unique:.6f}"

def usdt_coins_from_usdt(usdt: float) -> int:
    """Сколько коинов за данную сумму USDT."""
    return int(round(usdt / USDT_PER_100 * 100 / 2) * 2)  # округляем до чётного


# ─── HTTP: POST /create_usdt_invoice ──────────────────────────────────────────
async def http_create_usdt_invoice(request: web.Request) -> web.Response:
    ...
    if not user_id or not coins:
        return web.json_response({"error": "user_id and coins required"}, status=400, headers=CORS)

    try:
        coins = int(coins)
    except (ValueError, TypeError):
        return web.json_response({"error": "coins must be integer"}, status=400, headers=CORS)

    amount_str = usdt_unique_amount(coins)
    expires    = asyncio.get_event_loop().time() + USDT_INVOICE_TTL

    _usdt_pending[amount_str] = {
        "user_id": int(user_id),
        "coins":   coins,
        "expires": expires,
    }

    logger.info(f"USDT invoice created: user={user_id} coins={coins} amount={amount_str}")
...
# ─── TRONGRID: проверяем входящие транзакции ──────────────────────────────────
async def tron_check_loop():
    """Каждые 15 сек проверяем новые USDT TRC20 транзакции на наш кошелёк."""
    seen_txids: set = set()
    logger.info("TronGrid watcher started")

    while True:
        await asyncio.sleep(15)
        try:
            url = (
                f"https://api.trongrid.io/v1/accounts/{USDT_WALLET}"
                f"/transactions/trc20"
                f"?contract_address={USDT_CONTRACT}"
                f"&only_to=true&limit=20"
            )
            ...
            # Чистим протухшие инвойсы
            expired = [k for k, v in _usdt_pending.items() if v["expires"] < now]
            for k in expired:
                logger.info(f"USDT invoice expired: amount={k}")
                del _usdt_pending[k]
            ...
            for tx in data.get("data", []):
                ...
                # Сумма в USDT (делим на 10^6 — USDT у TRON 6 decimals)
                value_raw = int(tx.get("value", 0))
                amount    = value_raw / 1_000_000
                amount_str = f"{amount:.6f}"

                if amount_str not in _usdt_pending:
                    continue  # не наш инвойс

                invoice = _usdt_pending.pop(amount_str)
                ...
                logger.info(f"USDT paid! user={user_id} coins={coins} txid={txid}")
                ...
                if _app_bot:
                    try:
                        await _app_bot.send_message(
                            ...
                            text=(
                                f"✅ Оплата получена!\n"
                                f"💰 +{coins} 🟡 золотых коинов начислено\n"
                                f"💵 Получено: {amount:.2f} USDT\n"
                                f"\n🎮 Открой игру и наслаждайся!"
                            )
                        )
                    except Exception as e:
            ...
            logger.error(f"tron_check_loop error: {e}")
...
async def run():
    ...
    if PORT:
        await start_http()
        asyncio.create_task(tron_check_loop())
